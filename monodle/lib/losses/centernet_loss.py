...
def compute_depth_loss(input, target):
    depth_input = extract_input_from_tensor(
        input["depth"], target["indices"], target["mask_3d"]
    )
...
def compute_offset3d_loss(input, target):
    offset3d_input = extract_input_from_tensor(
        input["offset_3d"], target["indices"], target["mask_3d"]
    )
...
def compute_size3d_loss(input, target):
    size3d_input = extract_input_from_tensor(
        input["size_3d"], target["indices"], target["mask_3d"]
    )
...
def compute_heading_loss(input, target):
    ...
    heading_input_cls, heading_target_cls = (
        heading_input_cls[mask],
        heading_target_cls[mask],
    )
    if mask.sum() > 0:
        cls_loss = F.cross_entropy(
            heading_input_cls, heading_target_cls, reduction="mean"
        )
    else:
        cls_loss = 0.0
    ...
    heading_input_res, heading_target_res = (
        heading_input_res[mask],
        heading_target_res[mask],
    )
    cls_onehot = (
