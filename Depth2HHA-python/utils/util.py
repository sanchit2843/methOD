...
def filterItChopOff(f, r, sp):
    ...
    for i in range(np.size(ind)):
        ...
        y = J[i]
        clipInd = spInd[
            max(0, x - r) : min(H - 1, x + r), max(0, y - r) : min(W - 1, y + r)
        ]
    ...
    for i in range(f.shape[2]):
        ...
        tmp = signal.convolve2d(
            np.rot90(f[:, :, i], 2), np.rot90(np.rot90(B, 2), 2), mode="same"
        )
...
def invertIt(AtA):
    ...
    AtA_1[:, :, 0] = np.multiply(AtA[:, :, 3], AtA[:, :, 5]) - np.multiply(
        AtA[:, :, 4], AtA[:, :, 4]
    )
    AtA_1[:, :, 1] = -np.multiply(AtA[:, :, 1], AtA[:, :, 5]) + np.multiply(
        AtA[:, :, 2], AtA[:, :, 4]
    )
    AtA_1[:, :, 2] = np.multiply(AtA[:, :, 1], AtA[:, :, 4]) - np.multiply(
        AtA[:, :, 2], AtA[:, :, 3]
    )
    AtA_1[:, :, 3] = np.multiply(AtA[:, :, 0], AtA[:, :, 5]) - np.multiply(
        AtA[:, :, 2], AtA[:, :, 2]
    )
    AtA_1[:, :, 4] = -np.multiply(AtA[:, :, 0], AtA[:, :, 4]) + np.multiply(
        AtA[:, :, 1], AtA[:, :, 2]
    )
    AtA_1[:, :, 5] = np.multiply(AtA[:, :, 0], AtA[:, :, 3]) - np.multiply(
        AtA[:, :, 1], AtA[:, :, 1]
    )
...
def getYDirHelper(N, y0, thresh, num_iter):
    ...
    for i in range(num_iter):
        ...
        indF = abs(sim0) > np.cos(
            thresh
        )  # calculate 'floor' set.    |sin(theta)| < sin(thresh) ==> |cos(theta)| > cos(thresh)
